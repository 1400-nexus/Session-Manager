...
| [`docs/CROSS_TEAM_PENDING.md`](docs/CROSS_TEAM_PENDING.md) | What C is waiting on from A and B, one line per item, and the four queued changes each unblocks. Start here for the cross-team state. |
| [`docs/ANSWERS_FROM_C_002.md`](docs/ANSWERS_FROM_C_002.md) · [`docs/rx.proto.patch.md`](docs/rx.proto.patch.md) | C's reply to B on the shm bitmap (no), `SessionOpen.file_size` (yes, field 9), the `PurgeSession` triggers, and the exact proto diff. Referent for the queued proto + shm-header work — none of it landed yet. |
